...
Use this skill as the Route 1 workbench for 3GPP standards research. Keep the existing project functionality intact: the CLI, workflow files, source notes, MCP/RAG/GraphRAG adapters, TDoc directories, tool registry, online search recipes, taxonomy config, and report templates are all part of the skill's usable resources.
...
- For the full system contract: `workflows/research-system.md`
- For tool orchestration: `workflows/tool-orchestration.md`
- For configurable task/spec/query rules: `config/research-taxonomy.json`
- For tool roles: `tools/registry.json`
- For online source discovery: `sources/online-search-playbook.md`, `sources/search-recipes.json`
- For rendered report structure: `templates/research-report.md`
...
The research CLI must load domain rules and report shape from project assets, not from embedded Python knowledge:

- task classification, spec hints, query expansion, ranking, and claim templates: `config/research-taxonomy.json`
- report layout: `templates/research-report.md`
- tool roles and orchestration intent: `tools/registry.json`, `workflows/tool-orchestration.md`
- online source discovery: `sources/online-search-playbook.md`, `sources/search-recipes.json`
...
2. `tools/registry.json` and `workflows/tool-orchestration.md` to decide which tool role is needed.
3. `scripts/3gpp_research.py research` for one-click deep research, or lower-level fetch, parse, build-db, search, relations, verify when manual control is needed.
4. `mcp/` notes and configured MCP wrappers for spec discovery, terminology, implementation requirements, or patent background.
5. `rag/` notes or configured RAG tools for clause/procedure/IE retrieval.
6. `graphrag/` assets for TS -> CR -> TDoc -> meeting/company/release relationships.
7. `sources/` official 3GPP Portal, FTP, Forge, ETSI entry points.
8. `sources/online-search-playbook.md` and `sources/search-recipes.json` when local material is insufficient.
